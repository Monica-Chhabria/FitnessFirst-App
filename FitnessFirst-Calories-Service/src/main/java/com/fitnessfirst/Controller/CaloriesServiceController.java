...
public class CaloriesServiceController {
	 ...
	 * +authenticationRequest.getPassword()); try {
	 ...
	 * RequestMethod.POST) public String createProducts(@RequestBody Product
	 * product) { HttpHeaders headers = new HttpHeaders();
	 * headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
	 * HttpEntity<Product> entity = new HttpEntity<Product>(product,headers);
	 * 
	 * return restTemplate.exchange( "http://localhost:8080/products",
	 * HttpMethod.POST, entity, String.class).getBody(); String fooResourceUrl =
	 ...
	 */
	/*
	 * @GetMapping("/") public void getnutrients() { // TODO Auto-generated
	 * method stub System.out.println("inside getnutrients"); }
	 */
	@PostMapping("/saveFood")
	public ResponseEntity<?> saveFoodDetails(@RequestBody FoodDetails foodDetails) {
		HttpHeaders headers = new HttpHeaders();
		headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
		HttpEntity<String> entity = new HttpEntity<String>(headers);
		final String uri = env.getProperty("userdetailsfetchuser.url");
		UserDetail userDetail = restTemplate
				.exchange(uri + foodDetails.getUsername(), HttpMethod.GET, entity, UserDetail.class).getBody();
		...
		// System.out.println("outside fooddetails2");
	...
	public ResponseEntity<?> getTotalCaloriesPerMeal(@PathVariable("username") String email,
			@PathVariable("meal") String meal) throws JSONException {
	...
	public ResponseEntity<?> findAllCalPerMealPerDay(@PathVariable("username") String email,
			@PathVariable("meal") String meal) throws JSONException {
		List<FoodDetails> fooddetails = this.foodDetailsService.findAllCalPerMealPerDay(email, meal);
		List<FoodDetails> foodDetails2 = fooddetails.stream().map(fooddetail -> {
			fooddetail.setCalories(fooddetail.getCalories().setScale(2, BigDecimal.ROUND_DOWN));
			return fooddetail;
	...
	public ResponseEntity<NutrientOutput> getNutrientService(@PathVariable("food") String food) {
		...
		/*
		 * I suggest using one of the exchange methods that accepts an
		 * HttpEntity for which you can also set the HttpHeaders. (You can also
		 * specify the HTTP method you want to use.)
		 * 
		 * For example,
		 * 
		 * RestTemplate restTemplate = new RestTemplate(); HttpHeaders headers =
		 * new HttpHeaders();
